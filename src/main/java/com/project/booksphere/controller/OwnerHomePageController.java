...
public class OwnerHomePageController implements Initializable {
    ...
    void navigateToUserPage(ActionEvent event) {
        navigateTo("/view/ManageUserPage.fxml");
    }

    @FXML
    void navigateDashboard(ActionEvent event) throws IOException {
        navigateTo("/view/DashboardOwner.fxml");
    }
    ...
    private void setDashboard() throws IOException {
        ownerBodyPane.getChildren().clear();
        AnchorPane load =  FXMLLoader.load(getClass().getResource("/view/DashboardOwner.fxml"));
        load.prefHeightProperty().bind(ownerBodyPane.widthProperty());
        load.prefHeightProperty().bind(ownerBodyPane.heightProperty());
        ownerBodyPane.getChildren().add(load);
    }
